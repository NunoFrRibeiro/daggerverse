...
func (t *Twitter) SendTweet(
	...
	var post *types.CreateInput
	if message.Text != "" {
		post.Text = gotwi.String(message.Text)
	}
	if message.DirectMessageDeepLink != "" {
		post.DirectMessageDeepLink = gotwi.String(message.DirectMessageDeepLink)
	}
	if !message.ForSuperFollowersOnly {
		post.ForSuperFollowersOnly = gotwi.Bool(message.ForSuperFollowersOnly)
	}
	if message.GeoPlaceID != "" {
		post.Geo.PlaceID = gotwi.String(message.GeoPlaceID)
	}
	if message.TaggedUserID != "" {
		if message.MediaIDs != nil {
			post.Media.MediaIDs = message.MediaIDs
		}
		post.Media.TaggedUserID = gotwi.String(message.TaggedUserID)
	}
	if message.PollDurationMinutes > 0 {
		if message.PollOptions != nil {
			post.Poll.Options = message.PollOptions
			post.Poll.DurationMinutes = &message.PollDurationMinutes
		}
	}
	if message.QuoteTweetID != "" {
		post.QuoteTweetID = gotwi.String(message.QuoteTweetID)
	}
	if message.InReplyToTweetId != "" {
		if message.ExcludedUsersIDsReply != nil {
			for _, ID := range message.ExcludedUsersIDsReply {
				post.Reply.ExcludeReplyUserIDs = append(post.Reply.ExcludeReplyUserIDs, ID)
			}
		}
		post.Reply.InReplyToTweetID = *gotwi.String(message.InReplyToTweetId)
	}
	if message.ReplySettings != "" {
		post.ReplySettings = gotwi.String(message.ReplySettings)
	}
